...
app.get("/", (req, res) => {
    const sql = "SELECT * FROM students"; // SQL query to fetch all students
...
app.post("/create", (req, res) => {
    const sql = "INSERT INTO students(name,email,marks,grade,city) VALUES (?)"; // SQL query to insert a new student
    const values = [
...
app.put("/update/:id", (req, res) => {
    const sql = "UPDATE students SET `name` = ?, `email` = ?, `marks` = ?, `grade` = ?, `city` = ? WHERE id = ?"; // SQL query to update student details
    const values = [
        req.body.name, // Get name from request body
        req.body.email, // Get email from request body
...
app.delete("/students/:id", (req, res) => {
    const sql = "DELETE FROM students WHERE id = ?"; // SQL query to delete a student
    const id = req.params.id // Get student ID from route parameters
...
// Start the server
app.listen(4000, () => {
    console.log("Server started on port 4000"); // Log server start message
})
